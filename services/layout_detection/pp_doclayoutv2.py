...
class PPDocLayoutV2Config(RTDetrConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Set reading_order_config before super().__init__() because transformers 5.x
        # runs validators (including to_dict()) inside the parent __init__.
...
class PPDocLayoutV2ForObjectDetection(RTDetrForObjectDetection):
    ...
    def __init__(self, config: PPDocLayoutV2Config):
        super().__init__(config)
        # transformers 5.x sets class_embed/bbox_embed on model.decoder directly
        # (not on self), so save them before we replace self.model.
        _class_embed = getattr(self, "class_embed", None) or self.model.decoder.class_embed
        _bbox_embed = getattr(self, "bbox_embed", None) or self.model.decoder.bbox_embed
        self.model = PPDocLayoutV2Model(config)
        self.model.decoder.class_embed = _class_embed
        self.model.decoder.bbox_embed = _bbox_embed
