...
def _seed_case(
    *,
    ...
    with session_factory() as session:
        ...
        if assigned_server_id == service.POOL_SERVER_ID:
            service.ensure_pool_server(session)
        job = Job(
            id=job_id,
...
def _claim(case: PostgresShardCase, server_id: str):
    with case.session_factory() as session:
        _set_timeouts(session)
        shard = service.claim_next_pending_shard(
            session,
            case.job_id,
            server_id,
        )
...
def test_postgres_two_pending_shards_claim_with_skip_locked():
    ...
    entered, release, primary = _run_with_paused_commit(
        case,
        lambda session: service.claim_next_pending_shard(
            session,
            case.job_id,
            case.server_ids[0],
        ),
...
def test_postgres_claim_key_share_prevents_terminal_update_deadlock(monkeypatch):
    ...
    def terminal_update():
        with case.session_factory() as session:
            ...
            return service.update_work_shard(
                session,
                case.shard_ids[0],
                WorkShardUpdateRequest(
                    status="succeeded",
...
def test_postgres_32_shards_8_workers_claim_and_succeed_without_deadlock():
    ...
    def worker(server_id: str) -> list[int]:
        ...
        while time.monotonic() < deadline:
            ...
            with case.session_factory() as session:
                _set_timeouts(session)
                result = service.update_work_shard(
                    session,
                    shard_id,
                    WorkShardUpdateRequest(
                        status="succeeded",
...
def test_postgres_progress_updates_on_different_shards_do_not_share_job_lock():
    ...
    entered, release, primary = _run_with_paused_commit(
        case,
        lambda session: service.update_work_shard(
            session,
            case.shard_ids[0],
            WorkShardUpdateRequest(
                status="running",
...
def test_postgres_at_cap_dual_claimers_fail_once():
    ...
    def second_claim():
        with case.session_factory() as session:
            _set_timeouts(session)
            second_started.set()
            return service.claim_next_pending_shard(
                session,
                case.job_id,
                case.server_ids[2],
            )
...
def test_postgres_no_candidate_reconciliation_persists_after_session_close(
    ...
    pool_server = service.POOL_SERVER_ID if use_pool_path else None
    case = _seed_case(
        max_attempts=1,
    ...
    try:
        if use_pool_path:
            ...
            with case.session_factory() as session:
                _set_timeouts(session)
                assert service.claim_next_pool_job(
                    session,
                    case.server_ids[1],
                ) is None
        else:
...
def test_postgres_stop_serializes_before_lease_exhaustion():
    ...
    entered, release, primary = _run_with_paused_commit(
        case,
        lambda session: service.request_stop(session, case.job_id),
    )
    second_pid: list[int] = []
    second_started = threading.Event()
    ...
    def exhaust():
        with case.session_factory() as session:
            ...
            second_started.set()
            result = service.reconcile_expired_shard_leases(
                session,
                job_id=case.job_id,
            )
            session.commit()
...
def test_postgres_reregister_and_reconcile_complete_without_deadlock(
    ...
    original_latest_attempt = workers_core._latest_current_shard_attempt
    ...
    def reconcile():
        with case.session_factory() as session:
            _set_timeouts(session)
            service.reconcile_expired_shard_leases(
                session,
                job_id=case.job_id,
            )
            session.commit()
...
def test_postgres_heartbeat_waits_for_server_before_reregistered_shards(
    ...
    def heartbeat():
        with case.session_factory() as session:
            ...
            return service.heartbeat_server(
                session,
                case.server_ids[0],
                ServerHeartbeatRequest(
                    status="busy",
