...
def _finalize_job_after_shard_change(
    ...
    *,
    ...
    """Finalize a Job while its row lock serializes all shard terminal paths."""
    ...
    job.failure_category = (
        failed_shard.failure_category or "shard_failed"
    )
    ...
    *,
    ...
    for candidate_job_id in job_ids:
        ...
        for shard in candidate_shards:
            ...
            parent_stopping = (
                job.stop_requested or job.status == "stopping"
            )
            if parent_stopping:
